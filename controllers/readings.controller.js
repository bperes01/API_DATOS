...
const getReadingsBySensor = async (req, res) => {
  try {
      ...
      //mostrar del lado del servidor lo que llega
      console.log('Info de la query', query);
...
const createReading = async (req, res) => {
  try {
    const { sensorId, value, unit } = mongoSanitize(req.body);;
    /*
...
const deleteReadingsBySensor = async (req, res) => {
  ...
  const getReadingsByTimeRange = async (req, res) => {
      try {
          ...
          const filter = {
              sensorId: mongoSanitize(sensorId),
